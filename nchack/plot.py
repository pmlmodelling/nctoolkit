...
def plot(self, log = False, panel = False):
    ...
    if n_points > 1 and len(self.variables) == 1:
        out = subprocess.run("cdo griddes " + self.current, shell = True, stdout=subprocess.PIPE, stderr = subprocess.PIPE)
        ...
        if self_max.values > 0 and self_min.values < 0:
            ...
            return self.to_xarray().hvplot.image(lon_name, lat_name, self.variables[0], dynamic = True,  logz = log, cmap = "viridis").redim.range(**{self.variables[0]:(-self_min.values, v_max)})
